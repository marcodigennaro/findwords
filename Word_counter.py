...
## Word counts 
## by M. Di Gennaro - 19/05/2017

## Requirements:
## NLTK - http://www.nltk.org/install.html
## use "sudo pip install -U nltk" or "conda install nltk" to install 
...
def print_top_results( sorted_list ):
    log_line     = '' 
    for index in range( min(int(words_to_print), len(sorted_list) )):
        tmp_word =  '"{}"'.format( sorted_list[index][0] )
        log_line += '{}{:25} {:5} times\n'.format( ' '*10, tmp_word, sorted_list[index][1]  )
    return( log_line )
...
def main():
  ...
  ## Create log file
  main_log  = open( os.path.join( main_dir, 'main_log.txt' ), 'w+' )
  ...
  log_list    = [ 'log_1w.txt', 'log_2w.txt', 'log_3w.txt' ] 
  for text_name in list_of_files:
      ...
      for word_list, label, tmp_dict in zip( all_counts_lists, label_list, dict_list ):
          ...
          #most_common_words[ text_name ] = ( len(reduced_items), words_list )
